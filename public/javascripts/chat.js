$(document).ready(function() {
    ...
    window.fromUser = $.cookie('user');//从 cookie 中读取用户名，存于变量 from
    window.toUser = 'all';//设置默认接收对象为"所有人"
    //发送用户上线信号
...
function flushUsers(users) {
    ...
    $("#list > li").dblclick(function() {
        //如果不是双击的自己的名字
        if ($(this).attr('alt') != fromUser) {
            //设置被双击的用户为说话对象
            toUser = $(this).attr('alt');
            //清除之前的选中效果
...
$(document).on("click","#say",function(){
    ...
    if (toUser == "all") {
        ...
        $("#contents").append('<div style="color:#00f" >你(' + now() + ')对 ' + toUser + ' 说：<br/>' + $msg + '</div><br />');
    }
    //发送发话信息
    socket.emit('say', {from: fromUser, to: toUser, msg: $msg});
    //清空输入框并获得焦点
